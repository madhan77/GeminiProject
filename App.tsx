...
import { generateCleanupScript } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleGenerateScript = useCallback(async () => {
    const selectedFolders = Object.entries(folderSelection)
      .filter(([, isSelected]) => isSelected)
      .map(([folderName]) => folderName);

    if (selectedFolders.length === 0) {
      setError('Please select at least one folder to clean up.');
      return;
    }
    ...
    try {
      const script = await generateCleanupScript(selectedFolders);
      setGeneratedScript(script);
    } catch (err) {
      setError('Failed to generate script. Please ensure your Gemini API key is configured correctly and try again.');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">1. Select Folders to Clean</h2>
            <p className="text-gray-600 dark:text-gray-400 mt-2">Choose which categories of emails you want to permanently delete.</p>
          </div>
          
          <FolderSelector selection={folderSelection} setSelection={setFolderSelection} />
